...
if grep -q "env: java" render.yaml; then
    echo "✅ Backend configured as Java service"
else
    echo "❌ Backend NOT configured as Java service"
    exit 1
fi

# Check frontend service type
if grep -q "env: static" render.yaml; then
    echo "✅ Frontend configured as static service"
else
    echo "❌ Frontend NOT configured as static service"
    exit 1
fi
...
# Check database name
if grep -q "name: ecompta-db" render.yaml; then
    echo "✅ Database name set to ecompta-db"
else
